...
void usage()
{
    mexPrintf("MRNE: MRNE method\n");
    mexPrintf("This Matlab-MEX function is for the minimum-norm solution of.\n");
    mexPrintf("linear systems Ax=b.\n");
    mexPrintf("  x = MRNE_NESSOR(A, b);\n");
    mexPrintf("  [x, relres, iter] = MRNE_NESSOR(A', b, tol, maxit);\n\n");
...
// MRNE method
void MRNE(const ccs *A, double *b, mwIndex maxit, double *iter, double *relres, double *x){

	double *p, *r, *s, *t, *u, *v, *Aei, *AC;
	...
	// Allocate s
	if ((s = (double *)mxMalloc(sizeof(double) * m)) == NULL) {
		mexErrMsgTxt("Failed to allocate s");
	}

	// Allocate t
	if ((t = (double *)mxMalloc(sizeof(double) * n)) == NULL) {
		mexErrMsgTxt("Failed to allocate t");
	}

	// Allocate u
	if ((u = (double *)mxMalloc(sizeof(double) * n)) == NULL) {
		mexErrMsgTxt("Failed to allocate u");
	}

	// Allocate v
	if ((v = (double *)mxMalloc(sizeof(double) * m)) == NULL) {
	...
	for (k=0; k<maxit; ++k) {
		...
		for (tmp=zero, j=0; j<n; ++j) tmp += u[j] * t[j];
		alpha = gmm / tmp;

		daxpy(&m, &alpha, p, &inc1, x, &inc1);

		tmp = -alpha;
		daxpy(&n, &tmp, t, &inc1, r, &inc1);
		...
		for (i=0; i<m; ++i) p[i] = s[i] + beta*p[i];
